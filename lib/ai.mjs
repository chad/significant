...
export function templateHeadline(ev) {
  ...
  if (ev.diffuse) {
    return {
      headline: `Mentions of ${name} are elevated across multiple stories`,
      dek: `In ${ev.lead.x} of the last ${ev.lead.n} posts (baseline ~${(ev.lead.p0 * ev.lead.n).toFixed(1)}), but the posts don't share one story` + (ev.cohTerm ? ` — largest common thread: “${ev.cohTerm}” (${Math.round(ev.coherence * 100)}%).` : "."),
    };
  }
  return {
...
export async function writeHeadlines(events) {
  ...
  const payload = events.map((ev) => ({
    ...
    diffuse: !!ev.diffuse,
    story_coherence: ev.coherence != null ? ev.coherence : null,
...
- "headline": 6–12 words, neutral newsroom register, no clickbait, no hashtags, no emoji. Name the one story the posts tell, plainly and confidently.
- VETO RULE: if the posts do NOT clearly tell ONE story — they span unrelated topics, or the term is incidental (e.g. "cars" appearing only inside a statistic about data centers) — do NOT write a hedge headline like "Mentions of X are elevated". Return {"id": "...", "incoherent": true} for that item instead. A vetoed item is dropped from the feed entirely; that is the honest outcome. Never use the phrase "Mentions of".
...
${JSON.stringify(payload, null, 1)}`;
  ...
  try {
    ...
    for (const ev of events) out.set(ev.id, templateHeadline(ev)); // call-level failure: keep the feed alive
    return out;
  }
  let parsed = false;
  try {
    ...
    for (const item of arr) {
      ...
      if (item.incoherent) { out.set(item.id, { incoherent: true }); continue; }
      if (item.headline) {
        // belt & suspenders: a hedge headline is a veto that lost its nerve
        if (/^mentions of/i.test(item.headline)) { out.set(item.id, { incoherent: true }); continue; }
        out.set(item.id, { headline: String(item.headline).slice(0, 140), dek: String(item.dek || "").slice(0, 240) });
      }
    }
  } catch (e) {
    console.error("[ai] headline parse failed:", e.message);
  }
  // Parse succeeded but an item was omitted: that's a veto that forgot to write home,
  // not a license to ship an "X is surging" template card. Parse failure: templates.
  for (const ev of events) {
    if (!out.has(ev.id)) out.set(ev.id, parsed ? { incoherent: true } : templateHeadline(ev));
  }
  return out;
}
