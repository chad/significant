...
import { extractTerms, scanTopic, gate, poissonTail, ALPHA } from "./stats.mjs";
...
async function load() {
  try {
    ...
    for (const ev of raw.events || []) events.set(ev.id, ev);
...
async function persist() {
  ...
  const out = {
    ...
    topics: Object.fromEntries([...topics].map(([name, t]) => [name, {
      ...
      posts: t.posts.map(({ terms, termDisplay, ...p }) => p),
    }])),
