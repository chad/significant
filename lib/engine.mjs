...
async function cycle(bootstrap = false) {
  ...
  // scan + gate
  const scans = new Map();
  for (const [name, t] of topics) scans.set(name, scanTopic(t.posts, now));
  const g = gate(scans);
  ...
  for (const e of g.events) {
    ...
    if (ev.diffuse && prev && !prev.diffuse && prev.headline) { ev.headline = null; ev.dek = null; }
    events.set(id, ev);
    if (!prev) {
      newborn.push(ev);
...
export function snapshot() {
  const now = Date.now();
  const evs = [...events.values()].sort((a, b) => (b.active - a.active) || (a.lead.p - b.lead.p) || (b.lastSeen - a.lastSeen));
  return {
