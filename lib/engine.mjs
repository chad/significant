...
import { checkCoverage, eventQuery } from "./press.mjs";
...
const PURGE_RE = process.env.PURGE_TERMS ? new RegExp(process.env.PURGE_TERMS) : null;
let vetoed = {}; // event id -> retry-not-before timestamp (12h veto memory)
...
async function load() {
  ...
  try {
    ...
    console.error(`[engine] STATE LOAD FAILED (${e.message}) — trying backup`);
    try {
      raw = JSON.parse(await readFile(STATE_FILE + ".bak", "utf8"));
      console.error("[engine] recovered from state.json.bak");
    } catch {
      console.error("[engine] no usable backup — starting fresh (this should be rare and loud)");
    }
  }
  try {
    ...
    // counter floor: counters can never claim fewer posts than are literally on disk
    // (protects the homepage from ever lying low after a partial state recovery)
    const stored = [...topics.values()].reduce((s, t) => s + t.posts.length, 0);
    if (counters.postsAnalyzed < stored) counters.postsAnalyzed = stored;
    console.log(`[engine] restored ${stored} posts, ${events.size} events, ${history.length} ledger entries`);
  } catch (e) {
    console.error("[engine] LOAD FAILED MID-WAY (state partially applied):", e && e.stack || e);
  }
...
async function persist() {
  ...
  // atomic: a reader (or an overlapping deploy's sandbox) must never see a half-written file
  const tmp = STATE_FILE + ".tmp";
  await writeFile(tmp, JSON.stringify(out));
  await rename(tmp, STATE_FILE);
  // hourly backup so a corrupted state never costs more than an hour of ledger
  if (Date.now() - lastBackup > 3600e3) {
    lastBackup = Date.now();
    await copyFile(STATE_FILE, STATE_FILE + ".bak").catch(() => {});
  }
}
...
async function cycle(bootstrap = false) {
  ...
  rollDay();
  // honest counting: only hypotheses whose topic window actually CHANGED this cycle
  // (re-testing an unchanged window every 60s is not a new test)
  if (due.length) {
    const polled = new Set(due.map((cfg) => cfg.name));
    let mNew = 0;
    for (const [name, s] of scans) if (polled.has(name)) mNew += s.m || 0;
    counters.termsTested += mNew;
    counters.termsToday += mNew;
  }
  lastGate = g;
  ...
  for (const e of g.events) {
    ...
    // runtime denylist + veto memory: retracted terms and Claude-vetoed events must
    // not re-admit themselves every cycle (which would also re-bill Claude each time)
    if (PURGE_RE && (PURGE_RE.test(e.lead.term) || PURGE_RE.test(e.lead.display || ""))) continue;
    if (vetoed[id] && now < vetoed[id]) continue;
    seen.add(id);
    ...
    if (!prev) {
      ...
      // same story, different mirrors: normalized text overlap catches wire copies
      // that share no URL (the Iran power-plant story passed twice via different
      // syndicated texts before this existed)
      const norm = (s) => s.toLowerCase().replace(/https?:\/\/\S+|@\S+/g, "").replace(/[^a-z0-9 ]/g, " ").replace(/\s+/g, " ").trim().slice(0, 90);
      const texts = new Set(receipts.map((r) => norm(r.text)));
      let parent = null;
      for (const old of events.values()) {
        if (old.type !== "burst" || old.id === id || !old.receipts) continue;
        const minLen = Math.max(1, Math.min(old.receipts.length, receipts.length));
        const uOverlap = old.receipts.filter((r) => r.url && urls.has(r.url)).length;
        const tOverlap = old.receipts.filter((r) => texts.has(norm(r.text))).length;
        if (uOverlap >= 3 || uOverlap / minLen >= 0.34 || tOverlap >= 3 || tOverlap / minLen >= 0.4) { parent = old; break; }
      }
      if (parent) {
        parent.active = true;
        parent.lastSeen = now;
  ...
  const needH = [...events.values()].filter((ev) => ev.active && !ev.headline).slice(0, 6); // rest next cycle — keeps the batch inside max_tokens
  if (needH.length) {
  ...
  // press subtraction — the differentiator. A statistically anomalous burst the
  // AP already published is just the wire amplified by mirror bots; the product is
  // the PRECURSOR: a burst with no mainstream coverage yet. Checked against Google
  // News + HN (keyless public indexes); unknown if unreachable (no badge, honestly).
  const needPress = [...events.values()]
    .filter((ev) => ev.active && ev.type === "burst" && !ev.coverage)
    .slice(0, 4);
  for (const ev of needPress) {
    const q = eventQuery(ev);
    if (!q) { ev.coverage = { at: now, status: "unknown", gn: 0, hn: 0 }; dirty = true; continue; }
    const r = await checkCoverage(q);
    ev.query = q;
    ev.coverage = { at: now, gn: r.gn, hn: r.hn, status: !r.ok ? "unknown" : r.covered ? "confirmed" : "precursor" };
    ev.nextCheckAt = ev.coverage.status === "precursor" ? now + 4 * 3600e3 : null;
    console.log(`[press] #${ev.topic} "${ev.lead.display || ev.lead.term}" → ${ev.coverage.status} (gn:${r.gn} hn:${r.hn})`);
    const h = history.find((x) => x.id === ev.id);
    if (h && !h.coverage) h.coverage = ev.coverage; // backfill rows pushed before classification existed
    dirty = true;
  }

  // earliness tracking: re-check live precursors every 4h for 72h; when the
  // mainstream catches up, record exactly how early we were
  const rechecks = [...events.values()]
    .filter((ev) => ev.coverage?.status === "precursor" && ev.pressLag == null && ev.nextCheckAt && now >= ev.nextCheckAt && now - ev.firstSeen < 72 * 3600e3)
    .slice(0, 3);
  for (const ev of rechecks) {
    const r = await checkCoverage(ev.query || eventQuery(ev));
    if (r.ok && r.covered) {
      ev.pressLag = now - ev.firstSeen;
      ev.coverage = { ...ev.coverage, at: now, gn: r.gn, hn: r.hn, coveredAt: now };
      ev.nextCheckAt = null;
      const h = history.find((x) => x.id === ev.id);
      if (h) { h.pressLag = ev.pressLag; h.coverage = ev.coverage; }
      console.log(`[press] mainstream caught up to #${ev.topic} "${ev.lead.display || ev.lead.term}" — we were ${(ev.pressLag / 3600e3).toFixed(1)}h early`);
    } else {
      if (r.ok) ev.coverage = { ...ev.coverage, at: now, gn: r.gn, hn: r.hn };
      ev.nextCheckAt = now + 4 * 3600e3;
    }
    dirty = true;
  }
  ...
  for (const ev of newborn) {
    if (!ev.headline) { const h = templateHeadline(ev); ev.headline = h.headline; ev.dek = h.dek; } // never a blank ledger row
    history.push({
      ...
      coverage: ev.coverage || null, pressLag: ev.pressLag ?? null, query: ev.query || null,
  ...
  // backfill ledger rows whose Claude headline arrived after the push
  for (const h of history.slice(-100)) {
    if (!h.headline) { const ev = events.get(h.id); if (ev?.headline) { h.headline = ev.headline; h.dek = ev.dek; dirty = true; } }
  }
...
export function getLedger() {
  const prec = history.filter((e) => e.coverage?.status === "precursor");
  const lags = prec.filter((e) => e.pressLag != null).map((e) => e.pressLag).sort((a, b) => a - b);
  return {
    launchedAt,
    lastPassAt,
    totals: { passedEver: counters.passedEver, postsAnalyzed: counters.postsAnalyzed, termsTested: counters.termsTested },
    scoreboard: {
      precursors: prec.length,
      laterCovered: lags.length,
      medianLagH: lags.length ? Math.round((lags[lags.length >> 1] / 3600e3) * 10) / 10 : null,
    },
    days,
