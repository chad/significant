...
async function load() {
  try {
    ...
    const purge = process.env.PURGE_TERMS ? new RegExp(process.env.PURGE_TERMS) : null;
    const keep = (term, headline) => !purge || !(purge.test(term || "") || purge.test(headline || ""));
...
async function cycle(bootstrap = false) {
  ...
  for (const e of g.events) {
    ...
    const ev = {
      ...
      diffuse: !!e.diffuse, coherence: round2(e.coherence || 0), cohTerm: e.cohTerm || null,
    ...
    // an event that turns out diffuse must not keep a story-shaped headline —
    // and diffuse events never get Claude at all: templates only, no narrativization
    if (ev.diffuse) {
      if (prev && !prev.diffuse && prev.headline) { ev.headline = null; ev.dek = null; }
      if (!ev.headline) { const h = templateHeadline(ev); ev.headline = h.headline; ev.dek = h.dek; }
    }
    events.set(id, ev);
    if (!prev) {
      newborn.push(ev);
  ...
  // Diffuse events are excluded — they already have honest template headlines.
  const needH = [...events.values()].filter((ev) => ev.active && !ev.headline && !ev.diffuse);
  if (needH.length) {
...
export function snapshot() {
  ...
  const evs = [...events.values()].sort((a, b) =>
    (b.active - a.active) || ((a.diffuse ? 1 : 0) - (b.diffuse ? 1 : 0)) || (a.lead.p - b.lead.p) || (b.lastSeen - a.lastSeen));
  return {
