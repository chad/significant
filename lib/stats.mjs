...
const STOP = new Set(("a,about,above,after,again,against,all,also,am,an,and,any,are,aren,as,at,be,because,been,before," +
  ...
  // clickbait adjectives — the vocabulary of headlines, never of a story
  "hidden,secret,secrets,amazing,incredible,essential,ultimate,perfect,simple,easy,popular,favorite,massive,major,huge," +
  "viral,shocking,stunning,epic,insane,crazy,wild,weird,bizarre,genius,brilliant,powerful,surprising,unexpected,tech," +
  // nameless business nouns — real stories name names
  "firms,companies,businesses,brands,makers,giants,customers,consumers,workers,employees,users,experts,analysts,investors," +
  // paywall/boilerplate & platform self-reference — the plumbing of publishing
  "premium,subscribe,subscription,subscriber,newsletter,register,membership,exclusive,sponsored,advertisement,medium,calm,source")
  .split(","));
...
export function extractTerms(text) {
  const clean = String(text || "")
    // @handles and emails ("Hoodline@flipboard.com", "@user@server") — attribution plumbing
    .replace(/\S*@\S+/g, " ")
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (!isCap(tokens[i]) || LEAD_SKIP.has(norm(tokens[i])) || STOP.has(norm(tokens[i]).replace(/'/g, ""))) continue;
    let run = [tokens[i]];
    let j = i + 1;
    // entity runs must respect stopwords too — "Sunday August" is a date, not an entity
    while (j < tokens.length && run.length < 4 && ((isCap(tokens[j]) && !STOP.has(norm(tokens[j]).replace(/'/g, ""))) || (["of", "the", "de", "von", "van"].includes(norm(tokens[j])) && isCap(tokens[j + 1] || "")))) {
      run.push(tokens[j]); j++;
    }
    if (run.length >= 2) {
...
export function gate(scans /* Map<topic, scan> */) {
  ...
  for (const [topic, terms] of byTopic) {
    ...
    for (const seed of terms) {
      ...
      // ≥60%: a coin-flip 50% turned out to admit two-story mashups ("firms" = Trump
      // cybercrime + China chips) and vocabulary vouching for vocabulary ("Hidden" via "Tech")
      if (coh.frac < 0.6) {
        noise.push({ topic, ...lead, structuralPass: false, failed: [`no coherent story — best companion term covers ${Math.round(coh.frac * 100)}% of posts (need ≥60%)`] });
        continue;
      }
