...
export function gate(scans /* Map<topic, scan> */) {
  ...
  for (const [topic, terms] of byTopic) {
    ...
    for (const seed of terms) {
      ...
      // story coherence: does this burst share a second term across its posts, or is the
      // lead an umbrella word covering unrelated stories? (deterministic — no AI here)
      const coh = coherence(lead, scan ? scan.candidates : []);
      const postIds = [...new Set(cluster.flatMap((c) => c.postIds))];
      events.push({ topic, lead, cluster, postIds, coherence: coh.frac, cohTerm: coh.term, diffuse: coh.frac < 0.5 });
    }
  }
